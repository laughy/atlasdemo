 ...
 * task:mergeDebugResource
 * dependon:awoCopyDebugSolib class:CopyAwoSolibTask-1
 * dependoned: awoProcessDebugResources    class ProcessAwoAndroidResources-1
 */
public class CachedMergeResources extends MergeResources {
    ...
    private String getDependencyMd5() {

        StringBuilder stringBuilder = new StringBuilder();

        if (awbBundle.getResolvedCoordinates().getVersion().endsWith("SNAPSHOT")) {
            return "";
        }
        stringBuilder.append(awbBundle.getResolvedCoordinates().toString());

        for (AndroidLibrary androidLibrary : awbBundle.getAndroidLibraries()) {
            if (androidLibrary.getResolvedCoordinates().getVersion().endsWith("SNAPSHOT")) {
                return "";
            }
            stringBuilder.append(androidLibrary.getResolvedCoordinates().toString());
        }
