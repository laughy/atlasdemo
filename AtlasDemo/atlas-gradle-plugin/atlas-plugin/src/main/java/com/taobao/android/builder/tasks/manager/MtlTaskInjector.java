                        ...
                        for (Task task : tasks) {
                            ...
                            for (Task before : beforeTasks) {
                                ...
                                project.getLogger().quiet(
                                    "[tasks] set task----- taskname:" +task.getClass().getName()+"---"+ task.getProject().getName() + ":" + task.getName() + "->"
                                        +"beforetaskname:" +before.getClass().getName()+"---"+ before.getProject().getName() + ":" + before.getName());
    ...
    protected List<Task> findTask(Class<Task> clazz, String variantName) {
        ...
        for (Task task : androidTasks) {
            if (task instanceof DefaultAndroidTask) {
                ...
                //自定义的task，都是以原有的task有一定以来关系，命名上存在类似，此处将自定义task找到
